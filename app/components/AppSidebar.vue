...
const chatsWithoutProjects = computed(() =>
...
function filterChats(startDays: number, endDays?: number) {
  return filterChatsByDateRange(
    chatsWithoutProjects.value,
    startDays,
    endDays
  ).map(formatChatItem)
}

const todayChats = computed(() => filterChats(-1, 1))
const lastWeekChats = computed(() => filterChats(1, 7))
const lastMonthChats = computed(() => filterChats(7, 30))
const olderChats = computed(() => filterChats(30))
...
<template>
  ...
  >
    <div v-if="chatsWithoutProjects.length > 0" class="overflow-y-auto p-4">
    ...
    <div v-else class="overflow-y-auto p-4">
      <UAlert title="No Chats" description="Create a new chat to get started" color="neutral" variant="soft" class="mt-2" />
      <UButton size="sm" color="neutral" variant="soft" icon="i-heroicons-plus-small" class="mt-2 w-full" @click="handleCreateChat">New Chat</UButton>
    </div>
  </aside>
</template>